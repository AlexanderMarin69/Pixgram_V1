using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace Pixgram_V1.Models
{
    public class Cart
    {
        internal const string customerCartKey = "customer_cart";

        private List<CartRow> _cartRows = new List<CartRow>();

        public virtual void AddProduct(FileUpload p, int quantity)
        {
            var cartRow = _cartRows.Where(x => x.FileUpload.Id.Equals(p.Id)).FirstOrDefault();
            if (cartRow == null)
            {
                _cartRows.Add(new CartRow
                {
                    FileUpload = p,
                    Quantity = quantity
                });
            }
            else
            {
                cartRow.Quantity += quantity;
            }
        }

        public virtual void RemoveCartRow(FileUpload p)
        {
            _cartRows.RemoveAll(x => x.FileUpload.Id.Equals(p.Id));
        }

        //public virtual decimal GetCartTotal()
        //{
        //    var total = _cartRows.Sum(x => x.Product.Price * x.Quantity);
        //    return total;
        //}

        public virtual void EmptyCart()
        {
            _cartRows.Clear();
        }

        public virtual IEnumerable<CartRow> CartRows => _cartRows;
    }
}
